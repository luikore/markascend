  ...
  class Macro
    ...
    def parse_video
      ...
      unless /\A\s*(?<width>\d+)x(?<height>\d+)\s+(?<url>.+)\z/ =~ content
        raise SyntaxError, 'can not parse \video content, should be "#{WIDTH}x#{HEIGHT} #{URL}"'
      end
      ...
      when /youtu\.?be/
        ...
        unless id = url[/(?<=watch\?v=)\w+/] || url[/(?<=embed\/)\w+/] || url[/(?<=youtu\.be\/)\w+/]
          raise SyntaxError, 'can not parse youtube id'
        end
      ...
      when /vimeo/
        unless id = url[/(?<=vimeo\.com\/)\w+/]
          raise SyntaxError, 'can not parse vimeo id, should use link like this "http://vimeo.com/#{DIGITS}"'
        end
        %Q|<iframe width="#{width}" height="#{height}" src="https://player.vimeo.com/video/#{id}" frameborder="0" allowFullScreen></iframe>|
      when /sm/
        unless id = url[/\bsm\d+/]
          raise SyntaxError, 'can not find "sm#{DIGITS}" from link'
        end
        %Q|<script src="https://ext.nicovideo.jp/thumb_watch/#{id}?w=#{width}&h=#{height}"></script>"|
      else
        raise SyntaxError, 'failed to parse video link, currently only youtube, vimeo and niconico are supported'
      end
    end
  end
end
